...
import { darkPaperTheme, paperTheme } from './theme';
...
const MainTabs = () => {
  ...
  const t = scheme === 'dark' ? darkPaperTheme : paperTheme;
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        headerShown: false,
        ...
        tabBarIcon: ({ color }) => <MaterialCommunityIcons name={tabIcons[route.name]} size={22} color={color} />
      })}
    >
...
export default function App() {
  const scheme = useColorScheme();
  const activeTheme = scheme === 'dark' ? darkPaperTheme : paperTheme;
  return (
