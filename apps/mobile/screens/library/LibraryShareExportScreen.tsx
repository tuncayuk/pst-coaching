...
const SCOPE_OPTIONS: Array<{ key: ExportScope; label: string; desc: string }> = [
  ...
  {
    key: 'highlights_and_notes',
    label: 'Vurgu + Notlar',
    desc: 'Vurgular ve yazdiklariniz eklenir.'
  }
];
...
const LibraryShareExportContent = ({ favoriteId, isOffline }: { favoriteId?: string; isOffline?: boolean }) => {
  ...
  const notes = getNotesForUser(user?.id).filter((n: any) => n.source_id === favorite?.item_id);
  ...
  return (
    <>
      <SectionCard title={'Paylasim ve Disa Aktarma'}>
        <PText variant="bodySmall" style={styles.sourceLabel}>
          Kaynak
        </PText>
        ...
        <View style={styles.statsRow}>
          <PChip compact style={styles.statChip}>
            {highlights.length} vurgu
          </PChip>
          <PChip compact style={styles.statChip}>
            {notes.length} not
          </PChip>
      ...
      <SectionCard title="Gizlilik Onayi (BR-09)">
        <PText variant="bodySmall" style={styles.privacyText}>
          Disa aktarilan icerik kisisel verilerinizi icermektedir. Paylasim oncesinde gizlilik politikasini okudugunuzu
          ve icerigi paylasmayi onayladiginizi dogrulayin.
        </PText>
        ...
        <View style={styles.consentRow}>
          ...
          {privacyAccepted && (
            <PText variant="labelSmall" style={styles.consentCheck}>
              Onay verildi
            </PText>
          )}
        </View>
          ...
          <View key={opt.key}>
            <View style={styles.scopeRow}>
              <View style={styles.scopeInfo}>
                ...
                <PText variant="bodySmall" style={styles.scopeDesc}>
                  {opt.desc}
                </PText>
              </View>
              <PButton
      ...
      <SectionCard title="">
        {exportDone ? (
          <View style={styles.doneBlock}>
            <PText variant="titleSmall" style={styles.doneText}>
              PDF olusturuldu!
            </PText>
            <PText variant="bodySmall" style={styles.doneDesc}>
              Dosya paylasim menusu araciligiyla iletebilirsin.
            </PText>
            <PButton mode="contained" style={styles.exportBtn} onPress={() => navigation.goBack()}>
              Tamam
            </PButton>
          </View>
        ) : exporting ? (
          <View style={styles.progressBlock}>
            <PText variant="bodySmall" style={styles.exportingText}>
              PDF hazirlaniyor...
            </PText>
...
export const LibraryShareExportScreen = ({ route }: { route?: { params?: { state?: ScreenState; id?: string } } }) => {
