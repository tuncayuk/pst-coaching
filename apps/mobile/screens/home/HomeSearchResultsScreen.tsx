...
const HomeSearchResultsContent = ({ isOffline, query }: { isOffline?: boolean; query?: string }) => {
  ...
  useEffect(() => {
    trackCtaTap('home.search_results', 'results_viewed', {
      query: query ?? '',
                ...
                <PCard.Content>
                  <View style={styles.cardHeader}>
                    <View style={[styles.typeChip, { backgroundColor: typeColor.bg }]}>
                      <PText style={[styles.typeChipText, { color: typeColor.text }]}>{group.title}</PText>
                    </View>
                  </View>
                ...
                <PCard.Actions>
                  <PButton
                    ...
                    onPress={() => {
                      ...
                      trackCtaTap('home.search_results', 'content_item_tapped', {
                        id: item.id,
                        type: group.title
                      });
...
export const HomeSearchResultsScreen = ({
  ...
  if (state === 'empty') {
    return (
      <ScreenLayout title="Arama Sonuclari" subtitle={query ? `"${query}" icin sonuc yok` : 'Sonuc bulunamadi'}>
        <StateMessage
  ...
  return (
    <ScreenLayout title="Arama Sonuclari" subtitle={query ? `"${query}" sonuclari` : 'Araman icin oneriler'}>
