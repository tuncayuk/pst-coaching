...
const contracts = [
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Sticky header shows day number and 23:59 deadline chip. Progress bar updates as user scrolls. Audio bar has play/pause and 3-speed selector (0.75x/1x/1.25x). Highlight toolbar offers Vurgula/Not Ekle/Favoriye Kaydet. Font size toggle cycles small/medium/large."
      },
      ...
      offline: { description: "Offline state shows cached content; audio and submit actions disabled." },
    },
    analytics: {
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Play/pause button active. Speed selector shows 0.75x/1x/1.25x options. Current-word highlight syncs with playback position. Background audio continues when app is backgrounded."
      },
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Long-press or toolbar tap reveals Vurgula/Not Ekle/Favoriye Kaydet options. Highlights persist per BR-12 offline storage rule. Source attribution shown on each highlight card."
      },
      empty: { description: "No highlights yet; prompt user to long-press text." },
      error: { description: "Save error with retry." },
      offline: { description: "New highlights queued for sync; existing highlights shown from cache." },
    },
    analytics: {
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Two guiding questions with multiline inputs. Word counter updates live. Draft auto-saves 1.5s after last keystroke with a brief indicator. Warning banner appears after 23:00. Submit disabled after 23:59. Emotion chip selector (single-select). Proceeds to preview with answers passed as params."
      },
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Content items listed with navigation. 08:00 and 23:59 deadline chips shown. Today completed banner displayed when all items done. When current hour < 8, locked state shows countdown to 08:00 and hides content list."
      },
      empty: { description: "No content scheduled for this day." },
      error: { description: "Load error with retry." },
      offline: { description: "Last-synced day content shown; lock state evaluated from device clock." },
    },
    analytics: {
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Steps displayed in ordered list. Only the active step has a 'Bu Adimi Tamamla' button. Completing a step unlocks the next. Progress fraction shown in header bar. When all steps done, celebration card appears with progress-saved message and CTA to comment screen."
      },
