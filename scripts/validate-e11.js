...
try {
  const inv = JSON.parse(
    fs.readFileSync(path.join(ROOT, "artifacts/ux/screen_inventory.json"), "utf8")
  );
...
screenFiles.forEach((fp) => {
  ...
  if (!fs.existsSync(abs)) { ko(name + " (missing)"); return; }
  const txt = fs.readFileSync(abs, "utf8");
  const hasExport = /export (const|function|default) /.test(txt);
  const nonAscii = txt.split("").some((c) => c.charCodeAt(0) > 127);
  if (!hasExport) { ko(name + " (no export)"); return; }
  if (nonAscii) { ko(name + " (non-ASCII chars)"); return; }
  ok(name);
});
