...
const EXPECTED = {
  ...
  'coach.fr_e12_03': {
    route: '/coach/client/content',
    screen: 'CoachContentTrackingScreen.tsx',
    ac: 4
  },
...
for (const [id, cfg] of Object.entries(EXPECTED)) {
  ...
  if (!fs.existsSync(fp)) {
    fail(cfg.screen + ' missing');
    continue;
  }
  const txt = fs.readFileSync(fp, 'utf8');
  if (!/export (const|function)/.test(txt)) {
    fail(cfg.screen + ' no export');
    continue;
  }
  const nonAscii = txt.split('').some(c => c.charCodeAt(0) > 127);
  if (nonAscii) {
    fail(cfg.screen + ' has non-ASCII chars');
    continue;
  }
  pass(cfg.screen);
}
