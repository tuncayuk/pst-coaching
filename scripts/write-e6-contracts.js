...
const contracts = [
  {
    ...
    uiStates: {
      loading: { description: "Skeleton cards shown for weekly-summary, progress-map, and achievements sections." },
      ready: {
        description: "Hero shows user name + streak count. Per-content-type progress section lists journey/workshop/module/ebook items with completion bars (AC-FR-E6-01-01). Streak chip + submission rate visible (AC-FR-E6-01-02). All progress values have accessibilityLabel for screen readers (AC-FR-E6-01-03). Quick-nav row links to EmotionalMap, Strengths, WeeklySummary, ReportExport sub-screens.",
      },
      empty: { description: "Empty state explains no completed content yet; CTA navigates to Discover." },
      error: { description: "Error card with retry action; last cached snapshot shown if available." },
      offline: { description: "Last-synced progress shown; write actions (review) disabled with indicator." },
    },
    analytics: {
  ...
  },
  {
    ...
    purpose: "Implements FR-E6-02: Duygusal harita — 14/30-day period selector, legend, disclaimer, color-blind support.",
    ...
    uiStates: {
      ...
      ready: {
        description: "Toggle row lets user switch between 14-day and 30-day views (AC-FR-E6-02-01). Legend card shows Sakin/Netlik/Gergin labels with color swatches AND pattern/icon fallbacks for color blindness (AC-FR-E6-02-04). Mood grid uses both color and icon to represent emotional state. Disclaimer banner reading 'Bu bir teshis degildir' always visible below the map (AC-FR-E6-02-03). Mood summary cards with percentages.",
      },
    ...
    analytics: {
      ...
      events: [
        "progress_emotional_map_viewed",
        "progress_emotional_period_changed",
        "progress_emotional_map_refreshed",
      ],
    },
  ...
  },
  {
    ...
    purpose: "Implements FR-E6-03: Guclu ve gelisim alanlari — separate cards, concrete suggestions, navigation to content.",
    entryPoints: ["progress.dashboard"],
    inputs: [
      { name: "userId", type: "uuid", required: true, source: "session" },
    ],
    ...
    analytics: {
      ...
      events: [
        "progress_strengths_viewed",
        "progress_strength_suggestion_applied",
        "progress_personal_plan_created",
      ],
    },
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Weekly bar chart shows daily session counts. Trend card highlights positive trend area, challenge area, and one actionable suggestion (AC-FR-E6-04-01). Completed content list shows each finished item with type badge and completion date (AC-FR-E6-04-02).",
      },
      empty: {
        description: "Empty state shown when no content completed this week — with explanation message and CTA (AC-FR-E6-04-03).",
      },
      error: { description: "Error card with retry." },
  ...
  },
  {
    ...
    purpose: "Implements FR-E6-05: Icerik bitis degerlendirmesi — evaluation form after content, deferral, next suggestion.",
    ...
    uiStates: {
      ...
      ready: {
        description: "Evaluation form shown after completing a journey/workshop/module (AC-FR-E6-05-01). Star rating (1-5) and optional comment field. 'Daha Sonra' button defers the review (AC-FR-E6-05-02). On submit: answers saved and next content suggestion shown (AC-FR-E6-05-03).",
      },
  ...
  },
  {
    ...
    uiStates: {
      ...
      ready: {
        description: "Report summary card shows total sessions, completed items, streak, and top themes (AC-FR-E6-06-01). 'Raporu Indir' downloads a PDF (AC-FR-E6-06-02). 'Paylas' shows privacy consent modal first (BR-09), then native share sheet (AC-FR-E6-06-03). Format selector chips (PDF/Ozet) available.",
      },
      empty: { description: "No data for report yet; CTA to complete content." },
      error: { description: "Export error with retry." },
      offline: { description: "Download and share disabled offline; report summary visible from cache." },
    },
    analytics: {
