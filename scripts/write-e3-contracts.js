...
const contracts = {
  'subscription.fr_e3_01': {
    ...
    purpose: 'FR-E3-01: Lists Bireysel (1), Aile (5), Grup (10) plans with seat limits and feature comparison. Selected plan highlighted visually with checkmark. Devam Et CTA activates when a plan is chosen (AC-FR-E3-01-01/02/03). Offline: cached plan list, buy disabled.',
    ...
    uiStates: {
      ...
      ready: {description:'Plan cards with name, seat limit, price, feature list; selected plan has colored border and checkmark badge'},
    ...
    analytics: {
      ...
      events: ['plan_comparison_viewed','plan_card_selected','plan_comparison_continue_tapped','plan_comparison_retry_tapped']
    },
  ...
  'subscription.fr_e3_02': {
    ...
    purpose: 'FR-E3-02: Opens platform (Apple/Google) IAP sheet. Server-side validation grants entitlement immediately (BR-08). Shows order summary with plan, price, coupon field. Handles purchase error with clear user message (AC-FR-E3-02-01/02/03/04). Offline: purchase disabled, show offline notice.',
    ...
    uiStates: {
      ...
      ready: {description:'Order summary card (plan name, price, coupon field), platform IAP button, payment method section'},
    ...
    analytics: {
      ...
      events: ['checkout_viewed','checkout_coupon_applied','checkout_purchase_tapped','checkout_purchase_success','checkout_purchase_error']
    },
  ...
  'subscription.fr_e3_03': {
    ...
    purpose: 'FR-E3-03: Lists add-ons with price, active/passive status and toggle. Ek Kisi add-on shown only for Aile/Grup plans (AC-FR-E3-03-03). Plan Uyesi cannot purchase; only Plan Sahibi can act (AC-FR-E3-03-04). Offline: list visible, actions disabled.',
    ...
    uiStates: {
      ...
      ready: {description:'Add-on cards with name, description, price badge, Aktif/Pasif chip, toggle button. Owner sees full actions; member sees read-only.'},
  ...
  'subscription.fr_e3_04': {
    ...
    purpose: 'FR-E3-04: Student discount verification flow. Privacy notice shown first (AC-FR-E3-04-03). Two methods: school email or document upload (AC-FR-E3-04-01). Success applies 50% discount (AC-FR-E3-04-02). Annual re-verification required; expiry warning shown (AC-FR-E3-04-04). Offline: status visible, actions disabled.',
    entryPoints: ['subscription.overview'],
    inputs: [
      {name:'userId',type:'uuid',required:true,source:'session'},
      {name:'verificationStatus',type:'string',required:false,source:'student_discount_entity'}
    ],
    dataDependencies: ['Subscription'],
    uiStates: {
      loading: {description:'Skeleton verification status card and step list'},
      ready: {description:'Privacy notice card, verification status badge, method selection (email/document), step list with numbered items, expiry warning if within 30 days of annual renewal'},
    ...
    analytics: {
      ...
      events: ['student_discount_viewed','student_discount_method_selected','student_discount_started','student_discount_status_checked','student_expiry_warning_shown']
    },
  ...
  'subscription.fr_e3_05': {
    ...
    uiStates: {
      ...
      ready: {description:'Plan summary (name, renewal date, seat gauge), change plan CTA, cancellation effects list, cancel CTA routing to platform'},
    ...
    analytics: {
      ...
      events: ['plan_management_viewed','plan_change_tapped','plan_cancel_tapped','plan_cancel_confirmed','plan_cancel_platform_redirect']
    },
  ...
  'subscription.fr_e3_06': {
    ...
    purpose: 'FR-E3-06: Shows seat occupancy gauge (X/Y dolu). Invite by email/phone, remove member actions. If limit reached, Ek Kisi add-on upsell shown (AC-FR-E3-06-03). Member role sees only own row (AC-FR-E3-06-04). Owner sees all seats with remove/invite controls.',
    entryPoints: ['subscription.overview'],
    inputs: [
      {name:'userId',type:'uuid',required:true,source:'session'},
      {name:'subscriptionId',type:'uuid',required:true,source:'subscription_entity'},
    ...
    uiStates: {
      ...
      ready: {description:'Seat gauge bar (X/Y), seat list with user name/role/status, invite button (owner only), remove button per active member, Ek Kisi upsell card if full'},
    ...
    analytics: {
      ...
      events: ['seat_management_viewed','seat_invite_tapped','seat_member_removed','seat_limit_reached_upsell_shown','seat_addon_upsell_tapped']
    },
  ...
  'subscription.fr_e3_07': {
    ...
    purpose: 'FR-E3-07: Shows payment transactions with date, amount, currency, status. Receipt detail viewable per transaction (AC-FR-E3-07-03). Restore Purchases triggers platform IAP restore (AC-FR-E3-07-02). Offline: cached transaction list, restore disabled.',
    entryPoints: ['subscription.overview'],
    inputs: [
      {name:'userId',type:'uuid',required:true,source:'session'},
      {name:'subscriptionId',type:'uuid',required:true,source:'subscription_entity'}
    ...
    uiStates: {
      ...
      ready: {description:'Grouped payment list (date, amount, currency, status chip), Makbuz button per row, Restore Purchases section, billing info update link'},
    ...
    analytics: {
      ...
      events: ['payment_history_viewed','payment_receipt_viewed','restore_purchases_tapped','restore_purchases_success','restore_purchases_error']
    },
