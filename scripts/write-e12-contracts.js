...
for (const id of Object.keys(ROUTES)) {
  ...
  const contract = {
    ...
    inputs: [
      ...
      ...(id !== 'coach.fr_e12_01' ? [{ name: 'clientId', type: 'uuid', required: true, source: 'navigation' }] : [])
    ],
    dataDependencies: DATA_DEPS[id],
    uiStates: {
      loading: {
        description: 'Skeleton placeholders shown while loading ' + FR_MAP[id] + ' data.'
      },
      ready: {
        description: FR_MAP[id] + ' flows are fully interactive with primary CTA available.'
      },
      empty: {
